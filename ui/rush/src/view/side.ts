...
export function puzzleBox(ctrl: Controller): VNode {
  ...
  return h('div.rush__side__metas', [
    puzzleInfos(data.puzzle),
    gameInfos(data.game, data.puzzle)
  ]);
}

function puzzleInfos(puzzle): VNode {
  return h('div.infos.puzzle', {
    ...
    h('p', h('strong', `Rating: ${puzzle.rating}`)),
    h('p', h('strong', `Played ${window.lichess.numberFormat(puzzle.attempts)} times`))
  ])]);
}

function gameInfos(game, puzzle): VNode {
  return h('div.infos', {
    attrs: dataIcon(game.perf.icon)
  }, [h('div', [
    h('p', [
      'From game',
      h('a', {attrs: {href: `/${game.id}/${puzzle.color}#${puzzle.initialPly}`}}, '#' + game.id),
    ]),
    h('p', [
      game.clock, ' • ',
      game.perf.name, ' • ',
